...
def get_rigs():
	...
	return [o for o in bpy.data.objects if o.type=='ARMATURE' and 'rain' in o.data]

def get_rig():
	"""If the active object is a cloudrig, return it."""
	# HACK: Coming back to Rain in 2022, this script is now breaking future CloudRig rigs,
	# so let's make it Rain specific.
	rig = bpy.context.pose_object or bpy.context.object
	if rig and rig.type == 'ARMATURE' and 'rain' in rig.data:
		return rig
...
class RigUI_Settings(RigUI):
	...
	def draw(self, context):
		...
		rig_props = rig.rig_properties
		layout.row().prop(rig_props, 'render_modifiers', text='Enable Render Modifiers')
